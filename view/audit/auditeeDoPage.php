...
function tabledata($clause){ 
     ...
     <input type="file" class="fileLoader" id="<?php echo 'userFile'.$checklist['checklistId'] ?>" onchange="fileUpload(<?php echo $checklist['checklistId'] ?>)" style="display: none;">
...
<?php echo $auditTitle?>
   ...
   <?php if($_SESSION['user_role'] == 'auditee' || $_SESSION['user_role']=='demo') {?>
                <div class="row">
                   
                <div class="col-md-5" style="margin-top: 10px;"><?php if($GLOBALS['workingStatus']!="perform pending") ?><button class="btn btn-danger" onclick="saveAllChecklists(allClauses,true)">Draft </button> </div>
               
                <!-- <div class="co1-md-2"></div> -->

                <div class="col-md-7" style="margin-top: 10px;"> <?php if($GLOBALS['workingStatus']!="perform pending") ?> <button  class="btn btn-success" data-spinner-color="#333"  onclick="saveAndChangeAuditCklStatus(allClauses,<?php echo $auditId ?>, '<?php echo $workingStatus ?>', false, <?php echo $GLOBALS['capa'] ?>)" >Respond</button> </div>
