...
public function getAllAssetsBasedOnGroup($userId, $userRole, $string){
    ...
    public function getAssetReview($assetId){
     ...
     public function getAssetDetails($assetId){
       $sql = 'SELECT concat(ucase(mid(a.name,1,1)),lcase(mid(a.name,2))) as name,concat(ucase(mid(a.status,1,1)),lcase(mid(a.status,2))) as status,a.retention_period as retentionPeriod, a.integrity as integrity,a.availability as availability,concat(ucase(mid(a.description,1,1)),lcase(mid(a.description,2))) as description,c.name as complianceName,a.start_date as start_date,a.at_origin as at_origin,a.info_moved as info_moved,a.end_date as end_date,a.review_date as review_date,a.confidentiality as confidentiality,a.personal_data as personal_data,a.sensitive_data as sensitive_data,a.customer_data as customer_data, concat(ucase(mid(u.last_name,1,1)),lcase(mid(u.last_name,2))) as ownerName,concat(ucase(mid(uu.last_name,1,1)),lcase(mid(uu.last_name,2))) as custodianName, concat(ucase(mid(uuu.last_name,1,1)),lcase(mid(uuu.last_name,2))) as ReviewerName,ag.name as assetGroup, concat(ucase(mid(d.name,1,1)),lcase(mid(d.name,2))) as departmentname,l.name as locationname FROM asset a,user u,user uu,user uuu,compliance c,bu_location l,bu_deparment d,asset_classification ac,asset_group ag,company comp WHERE a.location_id=l.id AND a.department_id=d.id AND a.compliance_id=c.id AND a.company_id=comp.id AND ag.id=a.asset_group AND a.classification=ac.id AND a.asset_owner=u.id AND a.asset_custodian=uu.id AND a.asset_reviewer=uuu.id AND a.id=?';
         $paramArray = array();
        $paramArray[] = $assetId;        
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql,'i',$paramArray);        
    }  
     public function getAssetDetails1($assetId){
        $sql = 'SELECT concat(ucase(mid(a.name,1,1)),lcase(mid(a.name,2))) as name,concat(ucase(mid(a.status,1,1)),lcase(mid(a.status,2))) as status,a.retention_period as retentionPeriod, a.integrity as integrity,a.availability as availability, c.name as complianceName,c.id as complianceId,concat(ucase(mid(u.last_name,1,1)),lcase(mid(u.last_name,2))) as ownerName,concat(ucase(mid(uu.last_name,1,1)),lcase(mid(uu.last_name,2))) as custodianName, concat(ucase(mid(uuu.last_name,1,1)),lcase(mid(uuu.last_name,2))) as ReviewerName,concat(ucase(mid(a.description,1,1)),lcase(mid(a.description,2))) as description,concat(ucase(mid(d.name,1,1)),lcase(mid(d.name,2))) as departmentname,l.name as locationname, a.confidentiality as confidentiality,a.personal_data as personal_data,a.sensitive_data as sensitive_data,a.customer_data as customer_data FROM asset a, compliance c, user u,user uu,user uuu,bu_location l,bu_deparment d WHERE a.id=? AND c.id=a.compliance_id AND a.asset_owner=u.id AND a.asset_custodian=uu.id AND a.asset_reviewer=uuu.id AND a.location_id=l.id AND a.department_id=d.id'; 
         $paramArray = array();
        $paramArray[] = $assetId;        
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql,'i',$paramArray);        
    ...
    public function getAllAssetsForOwner($userId){
     ...
     public function getAllAssetsForOwner1($userId){
        $sql = 'SELECT a.id as assetId, concat(ucase(mid(a.name,1,1)),lcase(mid(a.name,2))) as name, ag.name as assetGroup, a.retention_period as retentionPeriod, u.last_name as ownerName, c.name as companyName, a.status as status,a.company_id from asset a, company c, user u, asset_group ag where a.company_id = c.id and a.asset_owner = u.id and ag.id=a.asset_group and a.status ="identified" ORDER BY a.id DESC';
        $paramArray = array();
        $paramArray[] = $userId;        
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql);        
...
private function getAllAssetsForOwnerBasedOnGroup($userId,$string){
    ...
    public function getAllAssetsForAssessmentReturned($userId){
        $sql = 'SELECT a.id as assetId, concat(ucase(mid(a.name,1,1)),lcase(mid(a.name,2))) as name, ag.name as assetGroup, a.retention_period as retentionPeriod, u.last_name as ownerName, c.name as companyName, a.status as status from asset a,company c, user u, asset_group ag where a.company_id = c.id and a.asset_owner = u.id and ag.id=a.asset_group AND ((a.status="assessment returned") OR (a.status="assessed")) ORDER BY a.id DESC';
        $paramArray = array();
        $paramArray[] = $userId;        
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql, 'i', $paramArray);        
    ...
    public function getAllAssetsForReviewReturned($userId){
        $sql = 'SELECT a.id as assetId, concat(ucase(mid(a.name,1,1)),lcase(mid(a.name,2))) as name, ag.name as assetGroup, a.retention_period as retentionPeriod, u.last_name as ownerName, c.name as companyName, a.status as status from asset a, company c, user u, asset_group ag where a.company_id = c.id and a.asset_owner = u.id and ag.id=a.asset_group AND a.status="review returned" ORDER BY a.id DESC';
        $paramArray = array();
        $paramArray[] = $userId;        
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql, 'i', $paramArray);        
