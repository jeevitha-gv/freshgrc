...
class BcpmManager { 
   ...
   return $dbOps->cudData($sql,'sssiissii',$paramArray);
    ...
    public function getAllBcpmForPlan(){
        $sql='SELECT b.id,b.update_name,b.start_date,b.update_date_update,b.review_date,u.last_name,b.status FROM bcpm b,user u WHERE b.status="create" and b.approved_by=u.id order by b.id desc' ;
        $dbOps = new DBOperations();
    ...
    public function BcpmReport($id){
     ...
     public function getAllBcpmForMaintainence(){
        $sql='SELECT b.id,bp.scope,bp.policy,bp.overview,bp.assumption,u.last_name,b.status FROM bcpm b,user u,bcpm_plan bp WHERE b.id=bp.bcpm_id and b.status="implemented" and b.approved_by=u.id ORDER BY id DESC ';
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql);

    }
    public function getAllBcpmForexercise(){
        $sql='SELECT b.id,bp.scope,bp.policy,bp.overview,bp.assumption,u.last_name,b.status FROM bcpm b,user u,bcpm_plan bp WHERE b.id=bp.bcpm_id and b.status="maintainancedone" and b.approved_by=u.id ORDER BY id DESC ';
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql);
    ...
    public function getAllBcpmForReport(){
        $sql='SELECT b.id, b.update_name, b.start_date, b.review_date, u.last_name, b.update_office_location, b.status FROM bcpm b, user u WHERE status = "exercised" and b.approved_by=u.id ORDER BY id DESC';
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql);
