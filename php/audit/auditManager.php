...
public function getAllAuditsForPublish($userId, $userRole){
    ...
    private function getAllAvlAuditsForPublish(){
        $sql = 'SELECT a.id as auditId, a.created_date as date, concat(ucase(mid(a.title,1,1)),lcase(mid(a.title,2))) as title, compl.name as complianceName, a.audit_freq, a.audit_type as type, u.last_name as auditorName, c.name as companyName, a.status as status, compl.id as complianceId from audit a,company c, compliance compl, user u where (a.company_id = c.id and a.compliance_id = compl.id and (a.auditor = u.id or a.auditor=u.id)  and (a.status="published" or a.status="approved") and (a.audit_freq="once" or DATE(a.start_date)<=DATE(NOW()) or a.parent_audit=0))   ORDER BY a.id DESC';     
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql);        
    }    
    ...
    private function getAllAvlAuditsForCreate(){
        $sql = 'SELECT a.id as auditId, a.created_date as date, concat(ucase(mid(a.title,1,1)),lcase(mid(a.title,2))) as title, compl.name as complianceName, a.audit_freq, a.audit_type as type, u.last_name as auditorName, c.name as companyName, a.status as status, compl.id as complianceId from audit a,company c, compliance compl, user u where (a.company_id = c.id and a.compliance_id = compl.id and (a.auditor = u.id or a.auditor=u.id)  and a.status="create" and (a.audit_freq="once" or DATE(a.start_date)<=DATE(NOW()) or a.parent_audit=0))   ORDER BY a.id DESC';     
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql);        
    }    
    ...
    public function getAlluploadfiles(){
        $sql = 'SELECT a.id as auditId, a.created_date as date, a.title as title,a.audit_type as type,a.start_date as Start_Date,a.end_date as End_Date, a.audit_freq, a.status as status,au.file_name,au.url from audit a,audit_uploadfile au where a.id=au.auditId ORDER BY a.id DESC';     
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql);        
    }    
    ...
    public function getAllCompliances($companyId){
        $sql = 'SELECT * from compliance where (status = "published" or status= "analyzed" or status = "in_draft") and company_id=7';
        $dbOps = new DBOperations();
        $paramArray=array($companyId);
        return $dbOps->fetchData($sql,'i',$paramArray);
    }

    public function getAllAvailCompliance($start_from, $limit) {
        $sql = 'SELECT id,name,description FROM comp LIMIT ?,?';
        $dbOps = new DBOperations();
        $paramArray = array();
        $paramArray[] = $start_from;
        $paramArray[] = $limit;
