...
public function getAllPolicysForPolicyReviewer1($userId){
  ...
  $sql ='SELECT * FROM `policy` WHERE id=?';
    ...
    public function getAllPolicyExpired($date){
        $sql = 'SELECT p.id as policyId, p.title as title, pt.name as policy_type, u.last_name as policyName,p.effective_from as start_date, pp.name as policyprocedure, p.status as status from policy p, user u, policy_types pt, policy_procedure pp where p.owner = u.id AND p.policy_type=pt.id AND p.policy_procedure=pp.id AND p.effective_till < "' . $date . '"';     
        $dbOps = new DBOperations();
        return $dbOps->fetchData($sql);        
    }
